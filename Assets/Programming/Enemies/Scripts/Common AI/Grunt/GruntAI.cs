...
public class LightAttack
{
    ...
    /// </summary>
    [Header("Attacking")] public float lightAttackDamage = 20f;
    ...
    /// </summary>
    [Header("Box check for light attack")] public float boxCastDepth = 2f;
...
public class GruntAI : AIBase
{
    ...
    /// </summary>
    [Header("AI Detecting Player")] [SerializeField]
    ...
    [SerializeField] protected float speedWhileNextToPlayer = 0.4f;
    ...
    protected virtual void AlertedThinking()
    {
        ...
        if (Vector3.Distance(playerTarget.position, transform.position) < lightAttackClass.minDistanceForAttack)
        {
            ...
            if (Vector3.Distance(playerTarget.position, transform.position) < 1.55f || attacking)
                currentSpeed = speedWhileNextToPlayer; // PathTarget = transform.position;
            ...
            transform.rotation = Quaternion.RotateTowards(transform.rotation,
                Quaternion.LookRotation(
                    new Vector3(pathTarget.x, transform.position.y, pathTarget.z) - transform.position, transform.up),
    ...
    public virtual void LightAttackCheckAndDamage()
    {
        Collider[] HitObjects = Physics.OverlapBox(
            transform.position + (transform.forward * lightAttackClass.boxCastForwardOffset),
            new Vector3(lightAttackClass.boxCastLength, lightAttackClass.boxCastHeight, lightAttackClass.boxCastDepth) /
            2f,
